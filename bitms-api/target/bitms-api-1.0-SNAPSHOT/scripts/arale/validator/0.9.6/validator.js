...
define("arale/validator/0.9.6/rule", ["$", "i18n"], function (require, exports, module) {
    ...
    addRule('radio', /.*/);
    addRule('checkbox', /.*/);
    addRule('passportId', /^\d{18}$/, i18n.prop('validate.number'));
    ...
    addRule('fundPwd',/\S{6,}/,i18n.prop('validate.password'));
    ...
    addRule('numberOfDigits', function (options) {
        var element = options.element;
        var reg="/^-?\\d+(\\.\\d{0,"+options.maxLength+"})?$/";
        return eval(reg).test(Number(element.val()));
