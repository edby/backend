...
<script type="text/javascript">
    var explosionEntrustDataClsPosGrid;
    $(function () {
        explosionEntrustDataClsPosGrid = $('#explosionEntrustDataClsPosGrid').datagrid({
            url:'${ctx}/entrust/matchEntrustList/closeData',
            queryParams:{
                entrustRelatedStockinfoId:'<%=FundConsts.WALLET_BTC2USD_TYPE%>',
                tradeType:'<%=TradeEnums.TRADE_TYPE_MATCHTRADE.getCode()%>',tableName:'table'
            },
            fit: true,
            striped: true,
            rownumbers: true,
            ...
            frozenColumns:[[
                {
                    ...
                    field: 'accountId',
                    formatter: function (value, row, index) {
                       return (row.dealBalance-row.entrustAmt*row.entrustPrice).toFixed(12);
                    }
                }
            ]],
            columns: [[
    ...
    function searchExplosionEntrustFun() {
        explosionEntrustDataClsPosGrid.datagrid('load',
                $.serializeObject($('#searchExplosionEntrustClsPosForm'))
        );
    }
    function cleanExplosionEntrustFun() {
        ($('#searchExplosionEntrustClsPosForm input')).val('');
        explosionEntrustDataClsPosGrid.datagrid('load',{})
    }
    $(function(){
        ...
        $('#clopos_table_id').switchbutton({
            checked: false,
            onChange: function(checked){
                if(checked)
                {
                    $("#clopos_table_name").val("tableHis");
                }else
                {
                    $("#clopos_table_name").val("table");
                }
                explosionEntrustDataClsPosGrid.datagrid('load',
                    $.serializeObject($('#searchExplosionEntrustClsPosForm'))
                );
...
<div class="easyui-layout" data-options="fit:true,border:false">
    <div data-options="region:'north',border:false" style="height: 30px; overflow: hidden;background-color: #fff">
        <Form id ="searchExplosionEntrustClsPosForm">
            <table>
                <tr>
                    <th>交易对</th>
                    <td>
                    ...
                    <td>
                    <td align="left">
                        <input id="clopos_table_name" name="tableName" value="table" type="hidden" />
                        <input id="clopos_table_id" style="width: 100px;" class="easyui-switchbutton" data-options="onText:'历史数据',offText:'当前数据'" >
    ...
    <div data-options="region:'center',border:true">
        <table id="explosionEntrustDataClsPosGrid" data-options="fit:true,border:false"></table>
    </div>
</div>