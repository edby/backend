 ...
 */
package com.blocain.bitms.payment.eth.core;

import static com.blocain.bitms.payment.eth.util.ByteUtil.EMPTY_BYTE_ARRAY;
import static com.blocain.bitms.payment.eth.util.ByteUtil.ZERO_BYTE_ARRAY;
...
import com.blocain.bitms.payment.eth.crypto.ECKey;
import com.blocain.bitms.payment.eth.crypto.ECKey.ECDSASignature;
import com.blocain.bitms.payment.eth.crypto.ECKey.MissingPrivateKeyException;
import com.blocain.bitms.payment.eth.crypto.HashUtil;
import com.blocain.bitms.payment.eth.util.*;
...
public class Transaction
{
    ...
    public synchronized void rlpParse()
    {
        ...
        try
        {
            ...
            {
                if (!(rlpElement instanceof RLPItem)) throw new RuntimeException("Transaction RLP elements shouldn't be lists");
            }
    ...
    private void validate()
    {
        ...
        if (receiveAddress != null && receiveAddress.length != 0 && receiveAddress.length != ADDRESS_LENGTH) throw new RuntimeException("Receive address is not valid");
        ...
        {
            if (BigIntegers.asUnsignedByteArray(signature.r).length > HASH_LENGTH) throw new RuntimeException("Signature R is not valid");
            if (BigIntegers.asUnsignedByteArray(signature.s).length > HASH_LENGTH) throw new RuntimeException("Signature S is not valid");
            if (getSender() != null && getSender().length != ADDRESS_LENGTH) throw new RuntimeException("Sender is not valid");
        }
    }
    ...
    public String toString(int maxDataSize)
    {
        ...
        return "TransactionData [" + "hash=" + ByteUtil.toHexString(hash) + "  nonce=" + ByteUtil.toHexString(nonce) + ", gasPrice=" + ByteUtil.toHexString(gasPrice)
                + ", gas=" + ByteUtil.toHexString(gasLimit) + ", receiveAddress=" + ByteUtil.toHexString(receiveAddress) + ", sendAddress="
                + ByteUtil.toHexString(getSender()) + ", value=" + ByteUtil.toHexString(value) + ", data=" + dataS + ", signatureV="
                + (signature == null ? "" : signature.v) + ", signatureR=" + (signature == null ? "" : ByteUtil.toHexString(BigIntegers.asUnsignedByteArray(signature.r)))
    ...
    {
        return new Transaction(BigIntegers.asUnsignedByteArray(nonce), BigIntegers.asUnsignedByteArray(gasPrice), BigIntegers.asUnsignedByteArray(gasLimit), Hex.decode(to),
    ...
    {
        return new Transaction(BigIntegers.asUnsignedByteArray(nonce), BigIntegers.asUnsignedByteArray(gasPrice), BigIntegers.asUnsignedByteArray(gasLimit), Hex.decode(to),
                BigIntegers.asUnsignedByteArray(amount), null, chainId);
    }
}
