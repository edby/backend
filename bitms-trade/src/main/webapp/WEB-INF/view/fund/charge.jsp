                                    ...
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuListERC" style="width:100%;border:none;padding:0;margin:0;">
                                            <c:forEach items="${listCoin}" var="data">
                                                <button class="btn btn-block dropdown-toggle text-left" onclick="JumpURList('${data.stockType}','${data.remark}')" ${data.canRecharge eq 'no'?'disabled':''} type="button" style="border-radius:0;color:#151922;margin-top:0;border-bottom:1px solid #999;background-color:#eee;">
                                ...
                                <li class="list-group-item">
                                    <span style="display: inline-block;">
                                        <span class="bitms-bg3 cf p8 bitms-radius2 hand" id="copyAddr" style="display: inline-block;">
                                            <span>BTC</span>
                                            <span><%--充值地址--%><fmt:message key="fund.charge.rechargeAddr" /></span>
                                        </span>
                                        <span id="item_addr" class="p8 bitms-bg4" style="display: inline-block;"></span>
                                    </span>
                                </li>
                                <li class="list-group-item">
                                    <small><%--重要提示：系统将在收到2个网络确认后会为您进行入账处理。--%><fmt:message key="fund.charge.txtFirst" /></small><br />
                                </li>
                                <li class="list-group-item">
    ...
    $(function() {
        ...
        seajs.use([ 'pagination', 'template', 'moment','qrcode','clipboard' ], function(
            ...
            renderPage = new Pagination({
                url : "${ctx}/fund/charge/chargeList",
                elem : "#pagination",
                form : $("#chargeForm"),
                rows : 8,
                method : "post",
            ...
            function changeData(id) {
                ...
                var addr ='';
                if(index == 0){
                    addr = addrs.btcWalletAddr;
                }
                if(index == 1){
                    addr = addrs.ltcWalletAddr;
                }
                if(index == 2){
                    addr = addrs.ethWalletAddr;
                }
                if (addr != '' && addr != null) {
                    $("#qrcode_img").show();
                    $("#qrcode_img").html("");
                    ...
                    $("#qrcode_img").css("border","10px solid #1e222d");
                    $("#qrcode_img").css("background-color","#1e222d");
                }
            }
            changeData($('#stockinfoId').val());
