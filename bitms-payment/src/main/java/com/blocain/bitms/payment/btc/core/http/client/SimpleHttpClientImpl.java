...
import com.blocain.bitms.payment.btc.core.NodeProperties;
...
import com.blocain.bitms.payment.btc.core.http.HttpConstants;
...
public class SimpleHttpClientImpl implements SimpleHttpClient
{
    ...
    public String execute(String reqMethod, String reqPayload) throws HttpLayerException
    {
        ...
        try
        {
            ...
            LOG.debug("-- execute(..): '{}' response payload received for HTTP '{}' request with " + "status line '{}'",
                    ((respPayloadEntity == null) ? "null" : "non-null"), reqMethod, response.getStatusLine());
            return respPayload;
        }
        ...
        finally
        {
            if (response != null)
            {
                try
                {
                    LOG.debug("-- execute(..): attempting to recycle old HTTP response (reply to a " + "'{}' request) with status line '{}'", reqMethod,
    ...
    private HttpRequestBase getNewRequest(String reqMethod, String reqPayload) throws URISyntaxException, UnsupportedEncodingException
    {
        ...
        request.setURI(new URI(String.format("%s://%s:%s/", nodeConfig.getProperty(NodeProperties.RPC_PROTOCOL.getKey()),
                nodeConfig.getProperty(NodeProperties.RPC_HOST.getKey()), nodeConfig.getProperty(NodeProperties.RPC_PORT.getKey()))));
        ...
        LOG.debug("<< getNewRequest(..): returning a new HTTP '{}' request with target endpoint " + "'{}' and headers '{}'", reqMethod, request.getURI(),
    ...
    {
        if (authScheme.equals(HttpConstants.AUTH_SCHEME_NONE)) { return null; }
        if (authScheme.equals(HttpConstants.AUTH_SCHEME_BASIC)) { return new BasicHeader(HttpConstants.HEADER_AUTH,
    ...
    private String getCredentials(String authScheme)
    {
        ...
        else if (authScheme.equals(HttpConstants.AUTH_SCHEME_BASIC)) { return Base64.encodeBase64String(
                (nodeConfig.getProperty(NodeProperties.RPC_USER.getKey()) + ":" + nodeConfig.getProperty(NodeProperties.RPC_PASSWORD.getKey())).getBytes()); }
        throw new IllegalArgumentException(Errors.ARGS_HTTP_AUTHSCHEME_UNSUPPORTED.getDescription());
    }
    
    private CloseableHttpResponse checkResponse(CloseableHttpResponse response) throws HttpLayerException
    {
        LOG.debug(">> checkResponse(..): checking HTTP response for non-OK status codes & unexpected header values");
        // StatusLine statusLine = response.getStatusLine();
        // if ((statusLine.getStatusCode() >= 400)
        // && (statusLine.getStatusCode() <= 499)) { throw new HttpLayerException(Errors.RESPONSE_HTTP_CLIENT_FAULT, statusLine.toString()); }
        // if ((statusLine.getStatusCode() >= 500)
        // && (statusLine.getStatusCode() <= 599)) { throw new HttpLayerException(Errors.RESPONSE_HTTP_SERVER_FAULT, statusLine.toString()); }
        return response;
    }
}