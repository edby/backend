 ...
 */
package com.blocain.bitms.payment.eth.crypto;
/**
...
import static com.blocain.bitms.payment.eth.util.BIUtil.isLessThan;
import static com.blocain.bitms.payment.eth.util.ByteUtil.bigIntegerToBytes;
...
import com.blocain.bitms.payment.eth.config.Constants;
import com.blocain.bitms.payment.eth.crypto.jce.*;
import com.blocain.bitms.payment.eth.util.ByteUtil;
...
public class ECKey implements Serializable
{
    ...
    public ECKey(Provider provider, PrivateKey privKey, ECPoint pub)
    {
        ...
        else
        {
            throw new IllegalArgumentException(
                    "Expected EC private key, given a private key object with class " + privKey.getClass().toString() + " and algorithm " + privKey.getAlgorithm());
        }
        if (pub == null)
        {
    ...
    public static class ECDSASignature
    {
        ...
        public byte[] toByteArray()
        {
            ...
            return ByteUtil.merge(ByteUtil.bigIntegerToBytes(this.r), ByteUtil.bigIntegerToBytes(this.s), new byte[]{fixedV});
        }
    ...
    {
        if (input.length != 32) { throw new IllegalArgumentException("Expected 32 byte input to ECDSA signature, not " + input.length); }
    ...
    public static byte[] signatureToKeyBytes(byte[] messageHash, String signatureBase64) throws SignatureException
    {
        ...
        return signatureToKeyBytes(messageHash, ECDSASignature.fromComponents(Arrays.copyOfRange(signatureEncoded, 1, 33), Arrays.copyOfRange(signatureEncoded, 33, 65),
    ...
    {
        if (privKey == null) { throw new MissingPrivateKeyException(); }
        if (!(privKey instanceof BCECPrivateKey)) { throw new UnsupportedOperationException("Cannot use the private key as an AES key"); }
