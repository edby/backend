...
import com.blocain.bitms.payment.btc.core.domain.Info;
...
public class DaemonConfigurator extends AgentConfigurator
{
    ...
    {
        if (btcdProvider == null) { throw new IllegalArgumentException(Errors.ARGS_BTCD_PROVIDER_NULL.getDescription()); }
        return btcdProvider;
    }
    ...
    public boolean checkMonitorStates(Map<Notifications, Future<Void>> futures)
    {
        ...
        for (Notifications notificationType : Notifications.values())
        {
            ...
            {
                LOG.warn("-- checkMonitorStates(..): no active '{}' notification monitor detected " + "(reason: {})", notificationType.name(),
                        ((monitorHandle == null) ? "failure to launch" : "task terminated prematurely"));
                isAllActive = false;
            }
        }
        return isAllActive;
