...
public class MatchTradeController extends QuotationController
{
    ...
    public JsonMessage doMatchSellBtc(BigDecimal entrustAmt, BigDecimal entrustPrice, String entrustType, Long exchangePairMoney) throws BusinessException
    {
        ...
        checkAssetIsAciveTrade(principal.getId(),stockInfo.getTradeStockinfoId());
    ...
    public void checkAssetIsAciveTrade(Long accountId,Long stockinfoId )
    {
        ...
        if(accountWalletAsset.getStockinfoId().longValue() != FundConsts.WALLET_ETH_TYPE.longValue()
                && accountWalletAsset.getStockinfoId().longValue() != FundConsts.WALLET_BIEX_TYPE.longValue())
        {
            if(StringUtils.equalsIgnoreCase(accountWalletAsset.getIsActiveTrade(),"no"))
            {
                throw new BusinessException("Please enable the token trading first");
            }
        }
    }
