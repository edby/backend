...
/**
 *  杠杆现货交易中心  控制器
 * <p>File：LeveragedSpotController.java</p>
 * <p>Title: LeveragedSpotController</p>
 * <p>Description:LeveragedSpotController</p>
...
@Api(description = "杠杆现货交易中心")
public class LeveragedSpotController extends QuotationController
{
     ...
     */
    @RequestMapping(value = "/leveragedSpotTrade")
    @ApiOperation(value = "杠杆现货交易中心页面导航")
    public ModelAndView leveragedSpotTrade(String exchangePair) throws BusinessException
    {
        ...
        // 钱包账户
        AccountWalletAsset walletAsset = new AccountWalletAsset();
        walletAsset.setAccountId(principal.getId());
        walletAsset.setStockinfoId(stockInfo.getTradeStockinfoId());
        List<AccountWalletAsset> listAccountWalletAsset = accountWalletAssetService.findList(walletAsset);
        if(listAccountWalletAsset.size()>0)
        {
            walletAsset = listAccountWalletAsset.get(0);
        }else
        {
            walletAsset = new AccountWalletAsset();
            walletAsset.setAmount(BigDecimal.ZERO);
            walletAsset.setStockCode(stockInfo.getTradeAmtUnit());
        }
        // 理财账户
        AccountWealthAsset wealthAsset = new AccountWealthAsset();
        wealthAsset.setWealthAccountId(principal.getId());
        wealthAsset.setStockinfoId(stockInfo.getCapitalStockinfoId());
        wealthAsset.setRelatedStockinfoId(stockInfo.getCapitalStockinfoId());
        List<AccountWealthAsset> listAccountWealthAsset = accountWealthAssetService.findList(wealthAsset);
        if(listAccountWealthAsset.size()>0)
        {
            wealthAsset = listAccountWealthAsset.get(0);
        }else
        {
            wealthAsset = new AccountWealthAsset();
            wealthAsset.setWealthAmt(BigDecimal.ZERO);
            wealthAsset.setStockCode(stockInfo.getCapitalAmtUnit());
        }
        ModelAndView mav = new ModelAndView("spot/leveragedSpotTrade");
        ...
        mav.addObject("walletAsset", walletAsset);
        mav.addObject("wealthAsset", wealthAsset);
     ...
     */
    @ResponseBody
    @RequestMapping(value = "/leveragedSpotTrade/getAccountFundAsset", method = RequestMethod.GET)
