...
public final class CollectionUtils
{
    ...
    {
        if (!NumberUtils.isEven(items.length)) { throw new IllegalArgumentException(Errors.ARGS_COUNT_UNEVEN.getDescription()); }
    ...
    {
        if (containsNull(lists)) { throw new IllegalArgumentException(Errors.ARGS_CONTAIN_NULL.getDescription()); }
    ...
    {
        if (containsNull(lists)) { throw new IllegalArgumentException(Errors.ARGS_CONTAIN_NULL.getDescription()); }
        if (!equalsSize(lists)) { throw new IllegalArgumentException(Errors.ARGS_COUNT_UNEQUAL.getDescription()); }
        List<Object> mergedList = new ArrayList<Object>();
        if (lists.length > 0)
        {
    ...
    public static boolean equalsSize(List<? extends Object> ... lists)
    {
        ...
        for (List<? extends Object> list : lists)
        {
            ...
            {
                if (sizes.add(list.size())) { return false; }
            }
        }
        return true;
    }
    ...
    {
        if (containsNull(arrays)) { throw new IllegalArgumentException(Errors.ARGS_CONTAIN_NULL.getDescription()); }
