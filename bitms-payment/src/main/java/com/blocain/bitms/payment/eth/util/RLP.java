 ...
 */
package com.blocain.bitms.payment.eth.util;

import static com.blocain.bitms.payment.eth.util.ByteUtil.*;
...
public class RLP
{
    ...
    {
        // null item
        if ((data[index] & 0xFF) == OFFSET_SHORT_ITEM) { return (byte) (data[index] - OFFSET_SHORT_ITEM); }
        // single byte item
        if ((data[index] & 0xFF) < OFFSET_SHORT_ITEM) { return data[index]; }
        // single byte item
        if ((data[index] & 0xFF) == OFFSET_SHORT_ITEM + 1) { return data[index + 1]; }
        return 0;
    }
    ...
    private static int nextItemLength(byte[] data, int index)
    {
        ...
        // [0xc0, 0xf7]
        if ((data[index] & 0xFF) >= OFFSET_SHORT_LIST && (data[index] & 0xFF) <= OFFSET_LONG_LIST) { return (byte) ((data[index] & 0xFF) - OFFSET_SHORT_LIST); }
        // [0xb8, 0xbf]
        ...
        // [0x81, 0xb7]
        if ((data[index] & 0xFF) > OFFSET_SHORT_ITEM && (data[index] & 0xFF) <= OFFSET_LONG_ITEM) { return (byte) ((data[index] & 0xFF) - OFFSET_SHORT_ITEM); }
        // [0x00, 0x80]
        ...
        try
        {
            ...
            while (pos < endPos)
            {
                ...
                if ((msgData[pos] & 0xFF) > OFFSET_LONG_LIST)
                {
                    ...
                    fullTraverse(msgData, level + 1, pos + lengthOfLength + 1, pos + lengthOfLength + length, levelToIndex, index);
        ...
        try
        {
            ...
            while (pos < endPos)
            {
                ...
                if ((msgData[pos] & 0xFF) > OFFSET_LONG_LIST)
                {
                    ...
                    if (length > msgData.length - pos - lengthOfLength) { throw new RuntimeException("Parsed data lays outside of RLP length boundaries"); }
                    ...
                    fullTraverse(msgData, level + 1, pos + lengthOfLength + 1, pos + lengthOfLength + length + 1, levelToIndex, newLevelList);
                ...
                if ((msgData[pos] & 0xFF) >= OFFSET_SHORT_LIST && (msgData[pos] & 0xFF) <= OFFSET_LONG_LIST)
                {
                    ...
                    RLPList newLevelList = new RLPList();
                    newLevelList.setRLPData(rlpData);
                    if (length > 0) fullTraverse(msgData, level + 1, pos + 1, pos + length + 1, levelToIndex, newLevelList);
                    rlpList.add(newLevelList);
                    pos += 1 + length;
                    continue;
                }
    ...
    {
        if (isNullOrZeroArray(srcData)) return 0;
    ...
    {
        if (size == 0) { return new byte[]{(byte) OFFSET_SHORT_LIST}; }
    ...
    {
        if (elements == null) { return new byte[]{(byte) OFFSET_SHORT_LIST}; }
        int totalLength = 0;
