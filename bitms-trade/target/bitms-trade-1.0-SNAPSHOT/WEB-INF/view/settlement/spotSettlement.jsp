            ...
            <form class="clearfix" data-widget="validator" name="entrustForm" id="entrustForm">
                ...
                <div class="panel clearfix">
                    <div class="table-responsive">
                        <h4 class="text-success"><%--交割结算--%><fmt:message key='spot.query.settlement' /></h4>
                        <table class="table table-hover">
                            ...
                            <tr>
                                ...
                                <th><%--交割结算价格--%><fmt:message key='spot.query.settlementPrice' /></th>
                                <th><%--分摊基金分摊数量--%><fmt:message key='spot.query.allpcationFund' /><fmt:message key='spot.query.allpcationQuantity' /></th>
                                <th><%--穿仓用户亏损分摊数量--%><fmt:message key='spot.query.userLoss' /><fmt:message key='spot.query.allpcationQuantity' /></th>
                                <th><%--分摊比例--%><fmt:message key='spot.query.allpcationProportion' /></th>
                            </tr>
                            </thead>
...
<script id="list_tpl" type="text/html">
    ...
    <tr class="test">
        ...
        <td>{{$statusFlag $value.settlementType}}</td>
        <td>{{$Fix8Flag $value.settlementPrice}}</td>
        <td>{{$Fix8Flag $value.reserveAllocatAmt}}</td>
        <td>{{$Fix8Flag $value.wearingSharingLossesAmt}}</td>
        <td>{{$percentFlag $value.assessmentRate}}</td>
    </tr>
    {{/each}}
</script>
...
<script>
    ...
    seajs.use(['pagination','validator','template','moment'], function (Pagination,Validator,Template,moment) {
        ...
        template.helper('$percentFlag', function(flag) {
            return  parseFloat((flag*100)).toFixed(2)+'%';
        });
