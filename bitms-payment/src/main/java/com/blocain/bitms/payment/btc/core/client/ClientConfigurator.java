...
import com.blocain.bitms.payment.btc.core.common.AgentConfigurator;
...
public class ClientConfigurator extends AgentConfigurator
{
    ...
    {
        return EnumSet.of(NodeProperties.RPC_PROTOCOL, NodeProperties.RPC_HOST, NodeProperties.RPC_PORT, NodeProperties.RPC_USER, NodeProperties.RPC_PASSWORD,
    ...
    public String checkNodeVersion(Integer encodedVersion)
    {
        ...
        {
            if (nodeVersion.equals(supportedVersion)) { return nodeVersion; }
        }
        LOG.warn("-- checkNodeVersion(..): server version mismatch - client optimized for '{}'" + ", node responded with '{}'", Defaults.NODE_VERSIONS, nodeVersion);
        return nodeVersion;
    }
    ...
    public boolean checkNodeHealth(Block bestBlock)
    {
        ...
        {
            LOG.warn("-- checkNodeHealth(..): last available block was mined >{} hours ago; please " + "check your network connection",
                    ((currentTime - bestBlock.getTime()) / 3600));
            return false;
        }
        return true;
    }
    ...
    {
        if (extendedFormat == null) { throw new IllegalArgumentException(Errors.ARGS_NULL.getDescription()); }
        ...
        List<Object> separatedFormat = CollectionUtils.mergeInterlaced(splittedFormat, CollectionUtils.duplicate(".", 4));
